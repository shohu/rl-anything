...
- **memory: APEX-MEM A++ temporal validity + provenance** — APEX-MEM（arXiv:2604.14362）の追記型・有効期間・クエリ時解決の核心を既存スタックで実装。
  ...
  - `hooks/instructions_loaded.py`: `_emit_stale_memory_warnings()` — superseded/stale な memory ファイルを stdout に出力してソフト指示（token フィルタは Approach C に委ねる）
  ...
  - `scripts/lib/audit.py`: `build_temporal_memory_warnings()` — decay_days 超過 / superseded_at 過去の memory を WARN、全 sources が reflect 済みなら削除候補
...
### Added
- **fleet: user-approved tracked projects list** — `bin/rl-fleet` の scan 対象を `~/matsukaze-utils/*` 固定から、ユーザーが明示承認した PJ 群に切り替え。他の配置（`~/updater/`, `~/jomon/`, `~/games/` 等）にある PJ も fleet で一覧できるようになる。
...
### Fixed
- **`bin/rl-audit --growth --skip-rescore` の hang 解消 + fleet env_score 表示** (min-sys#86) — fleet が rl-anything PJ のみ `TIMEOUT` で surface する根本修正。原因 2 件:
...
### Changed
- **Repository を min-sys org → shohu user account に移行** — min-sys/rl-anything を archive し、shohu/rl-anything を新正式ロケーションに。GitHub の組織→ユーザー直接 transfer は権限上不可だったため、stale な shohu/rl-anything へ main を fast-forward push + 100 tags 同期で移行。`plugin.json` / `marketplace.json` / `README.md` インストール手順 / docs 全体の URL 参照を一括更新。旧 min-sys repo は read-only で保存
